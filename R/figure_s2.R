# Aetiologic trends by region in SSA
...
colour_region <- c("#CC544D","#7577D1","#767676") 

vd_region <-  ggplot() +
  geom_ribbon(data=df_full_vd, aes(x=year, ymin=upr, ymax=lwr,alpha="Extrapolated",fill=region)) +
  geom_ribbon(data=df_sub_vd, aes(x=year, ymin=upr, ymax=lwr,alpha="Interpolated",fill=region)) +
  geom_point(data = df_study %>% filter(symptom=="Vaginal discharge") %>% mutate(rti = factor(rti,levels=c("CS","BV","CA","CT","TV","NG","MG","None"))), 
             aes(x=year, y=adj_prev,colour=region), alpha=0.6, size=1.1, stroke=NA) +
  geom_line(data=df_full_vd, aes(x=year, y=est, linetype="Extrapolated",colour=region), 
            linewidth=0.5) +
  geom_line(data=df_sub_vd, aes(x=year, y=est, linetype="Interpolated",colour=region), 
            linewidth=0.5) +
  ...
  scale_fill_manual("", breaks = c("Central and Western Africa","Eastern Africa","Southern Africa"), values=colour_region) +
  scale_colour_manual("", breaks = c("Central and Western Africa","Eastern Africa","Southern Africa"), values=colour_region) +
...
ud_region <-  ggplot() +
  geom_ribbon(data=df_full_ud, aes(x=year, ymin=upr, ymax=lwr,alpha="Extrapolated",fill=region)) +
  geom_ribbon(data=df_sub_ud, aes(x=year, ymin=upr, ymax=lwr,alpha="Interpolated",fill=region)) +
  geom_point(data = df_study %>% filter(symptom=="Urethral discharge") %>% mutate(rti=factor(rti,levels=c("NG","CT","MG","TV","None"))), 
             aes(x=year, y=adj_prev,colour=region), alpha=0.6, size=1.1, stroke=NA) +
  geom_line(data=df_full_ud, aes(x=year, y=est, linetype="Extrapolated",colour=region), 
            linewidth=0.5) +
  geom_line(data=df_sub_ud, aes(x=year, y=est, linetype="Interpolated",colour=region), 
            linewidth=0.5) +
  facet_wrap(~rti, ncol=5) +
  theme_classic(base_size = 6) +
  my_theme() +
...
gu_region <-  ggplot() +
  geom_ribbon(data=df_full_gu, aes(x=year, ymin=upr, ymax=lwr,alpha="Extrapolated",fill=region)) +
  geom_ribbon(data=df_sub_gu, aes(x=year, ymin=upr, ymax=lwr,alpha="Interpolated",fill=region)) +
  geom_point(data = df_study %>% filter(symptom=="Genital ulcer") %>% mutate(rti=factor(rti,levels=c("HSV","HSV-2","TP","HSV-1","HD","LGV","None"))), 
             aes(x=year, y=adj_prev,colour=region), alpha=0.6, size=1.1, stroke=NA) +
  geom_line(data=df_full_gu, aes(x=year, y=est, linetype="Extrapolated",colour=region), 
            linewidth=0.5) +
  geom_line(data=df_sub_gu, aes(x=year, y=est, linetype="Interpolated",colour=region), 
            linewidth=0.5) +
  facet_wrap(~rti, ncol=5) +
  theme_classic(base_size = 6) +
  my_theme() +
...
plot_region <- (vd_region / ud_region / gu_region) + 
...
ggsave("./plots/figure_s2.png", plot_region, width=15.4, height=18, units="cm", dpi=700)
